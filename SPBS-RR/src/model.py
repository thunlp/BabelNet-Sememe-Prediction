...
class TransE:
    ...
    def launch_evaluation(self, session):
        ...
        for eval_triple in self.kg.test_triples:
            self.t_idx_head_prediction, self.t_idx_tail_prediction = session.run(fetches=[self.idx_head_prediction,
                                                                            self.idx_tail_prediction],
                                                                   feed_dict={self.eval_triple: eval_triple})
            eval_result_queue.put((eval_triple, self.t_idx_head_prediction, self.t_idx_tail_prediction))
        ...
        fout = open('sememePre_TransE_' + str(self.global_step.eval()) + '.txt', 'w', encoding = 'utf-8')
    ...
    def calculate_rank(self, in_queue, out_queue):
        while True:
            ...
            if idx_predictions is None:
                ...
                AP_value, AP_n_value, output_str = score.Get_AP_by_entity_id(eval_triple,idx_tail_prediction[::-1])
